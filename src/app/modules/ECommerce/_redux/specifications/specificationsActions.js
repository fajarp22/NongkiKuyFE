...
export const fetchSpecifications = (queryParams, productId) => dispatch => {
  ...
  if (!productId) {
    return dispatch(actions.specificationsFetched({ totalCount: 0, entities: null }));
  }
...
export const fetchSpecification = id => dispatch => {
  if (!id) {
    return dispatch(actions.specificationFetched({ specificationForEdit: undefined }));
  }
  ...
  return requestFromServer
    ...
    .then(response => {
      ...
      dispatch(actions.specificationFetched({ specificationForEdit: specification }));
    })
    .catch(error => {
