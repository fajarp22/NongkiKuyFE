...
export function SpecificationsDeleteDialog() {
  ...
  const deleteSpecifications = () => {
    ...
    dispatch(actions.deleteSpecifications(specsUIProps.ids)).then(() => {
      ...
      dispatch(actions.fetchSpecifications(specsUIProps.queryParams, specsUIProps.productId)).then(
        () => {
          specsUIProps.setIds([]);
          specsUIProps.onHide();
        }
  ...
  return (
    <Modal
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">Specifications Delete</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {!isLoading && <span>Are you sure to permanently delete selected specifications?</span>}
