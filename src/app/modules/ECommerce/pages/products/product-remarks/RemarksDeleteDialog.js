...
export function RemarksDeleteDialog() {
  ...
  const deleteRemarks = () => {
    dispatch(actions.deleteRemarks(remarksUIProps.ids)).then(() => {
      dispatch(actions.fetchRemarks(remarksUIProps.queryParams, remarksUIProps.productId)).then(
        () => {
  ...
  return (
    <Modal
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">Remarks Delete</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {!isLoading && <span>Are you sure to permanently delete selected remarks?</span>}
      ...
      <Modal.Footer>
        <div>
          ...
          <> </>
          <button type="button" onClick={deleteRemarks} className="btn btn-primary btn-elevate">
            Delete
          </button>
        </div>
      </Modal.Footer>
